...
class TCPHandler(StreamRequestHandler):
    def handle(self):
        #if args.all:
        #    weights_steer_file = 'weight/steer/all.hdf5'
        #    weights_drift_file = 'weight/drift/all.hdf5'
        #    logger.info("Loading {}...".format(weights_steer_file))
        #    model.load_weights(weights_file)
        #    model1 = create_model(model,'weights/all.hdf5')
        #    model1.load_weights(weights_drift_file)
        ...
        for line in self.rfile:
            ...
            #    weights_steer_file = 'weights/steer/{}.hdf5'.format(course)
            #    weights_drift_file = 'weights/drift/{}/hdf5'.format(course)
            #    logger.info("Loading {}...".format(weights_steer_file))
            #    logger.info("Loading {}...".format(weights_drift_file))
            #    model.load_weights(weights_file)
            #    model1=create_model_drift(model,weights_steer_file)

            if message.startswith("PREDICTFROMCLIPBOARD"):
                im = ImageGrab.grabclipboard()
                if im != None:
                    result  = model.predict(prepare_image(im), batch_size=1)[0]
                    prediction = np.argmax(result)
                    print(prediction)
                    #prediction = np.argmax(prediction[0], axis=1)
                    #prediction1= model1.predict(prepare_image(im), batch_size=1)[0]
...
if __name__ == "__main__":
    ...
    if args.all:
        #model.load_weights('C:/Users/Junyi Chen/Desktop/NeuralKart-master/weights/steer/rrd.hdf5')
        model.load_weights('C:/Users/Junyi Chen/Desktop/NeuralKart-master - Copy/weights/lr.hdf5')
        #model1 = create_model_drift(model,'C:/Users/Junyi Chen/Desktop/NeuralKart-master/weights/steer/rrd.hdf5')
        #model1.load_weights('C:/Users/Junyi Chen/Desktop/NeuralKart-master/weights/drift/rrd.hdf5')
