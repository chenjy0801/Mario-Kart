...
USE_REVERSE_IMAGES = True

def customized_loss(y_true, y_pred, loss='euclidean'):
	# Simply a mean squared error that penalizes large joystick summed values
	if loss == 'L2':
	   L2_norm_cost = 0.001
	   val = K.mean(K.square((y_pred - y_true)), axis=-1) + K.sum(K.square(y_pred), axis=-1) / 2 * L2_norm_cost
...
def load_training_data(track):
	...
	if track == 'all':
		...
		recordings = list(glob.iglob("C:/Users/Junyi Chen/Desktop/NeuralKart-master - Copy/recordings/LR/*/*"))
	for recording in recordings:
		filenames = list(glob.iglob('{}/*.png'.format(recording)))
		filenames.sort(key=lambda f: int(os.path.basename(f)[:-4]))


		steering = [int(line) for line in open(("{}/steering.txt").format(recording)).read().splitlines()]
		...
		for file, steer in zip(filenames, steering):
			...
			if USE_REVERSE_IMAGES and steer != 0:
				...
				if valid_reversed:
					...
					if(steer<=4):
						y_train.append(5-steer)
					else:
						y_train.append(19-steer)


				else:
					X_val.append(im_reverse_arr)
					if(steer<=4):
						y_val.append(5-steer)
					else:
						y_val.append(19-steer)
...
if __name__ == '__main__':
	...
	mkdir_p("C:/Users/Junyi Chen/Desktop/NeuralKart-master - Copy/weights")
	weights_file = "C:/Users/Junyi Chen/Desktop/NeuralKart-master - Copy/weights/lr.hdf5"
